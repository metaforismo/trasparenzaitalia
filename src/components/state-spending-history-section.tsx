...
export async function StateSpendingHistorySection() {
  try {
    ...
    const maxMonth = history.points.reduce(
      ...
      <section className={styles.section}>
        ...
        <div className={styles.metrics}>
          ...
          <div>
            ...
            <strong>{maxMonth ? compactEuro(maxMonth.monthlyPaid) : "—"}</strong>
            <small>{maxMonth ? maxMonth.monthName : "dato non disponibile"}</small>
          </div>
        </div>
        ...
        <div className={styles.methodology}>
          ...
          <div>
            <span>Acquisito</span>
            <strong>{new Intl.DateTimeFormat("it-IT", {
              dateStyle: "medium",
              timeStyle: "short",
              timeZone: "Europe/Rome",
            }).format(new Date(history.observedAt))}</strong>
          </div>
        </div>
      </section>
    );
